...
POLL_INTERVAL_SEC = 60  # Safe for free CoinGecko tier

def get_coingecko_prices():
    """Single optimized call: KVT + KAU + KAG + C1USD (Tether as proxy for stable USD)"""
    try:
        ...
        headers = {"User-Agent": "PM_Aggregator_TradingBot/2.4"}
        ...
        return {
            ...
            "c1usd_usd": float(data.get("tether", {}).get("usd", 1.0))  # C1USD reference via stablecoin
        }
...
def main_collector():
    print("🚀 Kinesis Arbitrage Collector v2.4 – C1USD added | All prices in USD")
    print("Data ready for KVT/KAU/KAG ratio analysis + future MA signals")

    while True:
        cg = get_coingecko_prices()
        now = datetime.now(UTC)

        # Core arbitrage ratios (cheapest KVT entry when ratios are low)
